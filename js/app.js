...
(function initPageTransitions() {
    ...
    function preloadPage(url) {
        if (pageCache.has(url) || url === '/' || url === 'index.html' || url === '/myweb/' || url === '/myweb/index.html') return;
        fetch(url)
            .then(response => {
                if (response.ok) {
    ...
    document.addEventListener('mouseover', (e) => {
        ...
        if (url === '/myweb/index.html' || url === '/myweb/') url = '/myweb/';
        preloadPage(url);
    }, { passive: true });
    ...
    document.addEventListener('click', (e) => {
        ...
        setTimeout(() => {
            ...
            loadPage()
                ...
                .catch(() => {
                    // 加载失败，回退到正常跳转
                    document.body.classList.remove('page-transition--out', 'page-loading');
                    if (loader && loader.parentNode) {
                        loader.remove();
                    }
...
function initSearch() {
    ...
    function navigateToKnowledge(page, title) {
        ...
        const urlMap = {
            nature: '/myweb/knowledge/nature.html',
            humanity: '/myweb/knowledge/humanity.html',
            region: '/myweb/knowledge/region.html',
            elective: '/myweb/knowledge/elective.html',
        };
        const targetUrl = urlMap[page] || '/myweb/index.html';
...
// ===== 知识点折叠/展开 + 全部展开/收起 + localStorage 持久化 =====

const COLLAPSE_STORAGE_KEY = 'geography-collapse-state';

/**
 * 获取 localStorage 中保存的折叠状态
 * @param {string} pageKey - 页面标识（如 nature, humanity）
 * 返回 Set，包含所有已折叠的 knowledge-point 的索引字符串 "sectionId-index"
 */
    ...
    sections.forEach(section => {
        ...
        points.forEach(point => {
            point.addEventListener('click', (e) => {
                ...
                // 如果点击的是标题内的链接或其他元素，不触发折叠
                if (e.target.closest('.knowledge-point__title a')) return;

                point.classList.toggle('collapsed');
            });
        });

        // 为每个章节添加"展开全部/收起全部"按钮
...
function initRegisterForm(form) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (email === DEMO_ACCOUNT.email) {
            setAuthMessage(message, '演示账号已内置，请前往<a href="/myweb/login.html" style="color:var(--color-primary-dark);font-weight:800;">登录页面</a>直接登录。', 'error');
            refresh();
            return;
        }
        // 调用后端注册 API
        try {
            ...
            if (data.success) {
                ...
                form.reset();
                refresh();
                setTimeout(() => { window.location.href = '/myweb/index.html?register=success'; }, 900);
            } else {
                setAuthMessage(message, data.message, 'error');
                refresh();
            }
        } catch (err) {
            setAuthMessage(message, '网络错误 — 请确保后端服务器已运行（在项目目录执行 node server.js 或双击 start.bat），然后通过 http://localhost:3000 访问网站。', 'error');
            refresh();
        }
    });
}
...
function initLoginForm(form) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (email === DEMO_ACCOUNT.email && password === DEMO_ACCOUNT.password) {
            ...
            setTimeout(() => { window.location.href = '/myweb/index.html?login=success'; }, 900);
            return;
        }

        // ---- 普通登录（需要后端验证码 + API）----
        if (!currentEmailCode) {
            setAuthMessage(message, '请先点击"获取验证码"生成验证码。', 'error');
            return;
        }
...
function checkAnchor() {
    ...
    if (hash.startsWith('#search:')) {
        ...
        // 解析标题（忽略 &expand=1 后缀）
        const titlePart = decodeURIComponent(hash.substring(8).replace(/&expand=1$/, ''));
        // 尝试滚动到匹配的知识点
        ...
        if (matchedPoint) {
            // 搜索跳转时只展开匹配的那个知识点，其余保持折叠
            if (expandFlag) {
                matchedPoint.classList.remove('collapsed');
            }

            // 滚动到匹配位置
            matchedPoint.scrollIntoView({ behavior: 'smooth', block: 'center' });
